...
def styled_button(parent, text, command, color=ACCENT, width=22, **kw):
    ...
    btn.bind("<Enter>", lambda e=None: btn.config(bg=_lighten(color)))
    btn.bind("<Leave>", lambda e=None: btn.config(bg=color))
    return btn
...
def ghost_button(parent, text, command, width=18, **kw):
    ...
    btn.bind("<Enter>", lambda e=None: btn.config(bg=BORDER, fg=TEXT, highlightbackground=ACCENT))
    btn.bind("<Leave>", lambda e=None: btn.config(bg=SURFACE2, fg=TEXT2, highlightbackground=BORDER))
    return btn
    ...
    def submit_answer(event=None):
        ...
        if live_cfg.get("save_history", True):
            try:
                ...
                show_toast(answer_box.winfo_toplevel(),
    ...
    def save_goals():
        try:
            ...
            if settings_saved:
                ...
                show_toast(frame.winfo_toplevel(), f"Goals updated: {d}/day, {w}/week (settings save skipped)", "warning", 3500)
            if _refresh_ref[0]:
                _refresh_ref[0]()
        except Exception as ex:
            messagebox.showerror("Error", str(ex))
